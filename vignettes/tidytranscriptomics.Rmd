...
[This Google doc](https://docs.google.com/document/d/1RpdYAy8sgu9Dxg-Jcv9BKiVXNt1dRKGf8_xtJjT1ctE/edit?usp=sharing) has all the links you need for this workshop. As it contains logins to the Cloud servers, it will be accessible during the workshop only.

* Material
* Menti for Background Question and Challenges Quiz
* RStudio cloud logins
* Feedback survey

### Audience Background

First let's get an idea of today's audience.

Please either open a tab in your browser or use your phone. Go go to Menti (www.menti.com) and type the code given the Google doc above.

Then please rate the following 1-5 (1=no/none, 5=yes/a lot)
...
This tidy SingleCellExperiment object can be interacted with using [SingleCellExperiment commands](https://bioconductor.org/packages/release/bioc/vignettes/SingleCellExperiment/inst/doc/intro.html) such as `assayNames`.
...
This one is not a multiple choice menti question.
```
...
Challenge: Without creating any variable, check if the variability of the 1st UMAP dimension when calculating 2 components (ncomponents = 2) is equal/more/less than when calculating 3 components.
Select one of the multiple choice options in www.menti.com (code in Google doc).
```
...
```{challenge_6 class.source="challenge"}
...
-   `tidyseurat`, similarly, enables us to visualise and manipulate a Seurat object as if it were a tidy data frame and can also be used with any `Seurat` compatible method
